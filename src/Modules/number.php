...
add_action( 'wpcf7_init', 'wpcf7cms_add_form_tag_number', 10, 0 );

function wpcf7cms_add_form_tag_number() {
	wpcf7_add_form_tag( array( 'number', 'number*', 'range', 'range*' ),
		'wpcf7cms_number_form_tag_handler',
		array(
			'name-attr' => true,
		)
	);
...
function wpcf7cms_number_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
...
add_filter( 'wpcf7_validate_number', 'wpcf7cms_number_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_number*', 'wpcf7cms_number_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_range', 'wpcf7cms_number_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_range*', 'wpcf7cms_number_validation_filter', 10, 2 );

function wpcf7cms_number_validation_filter( $result, $tag ) {
	$name = $tag->name;
...
add_filter( 'wpcf7_messages', 'wpcf7cms_number_messages', 10, 1 );

function wpcf7cms_number_messages( $messages ) {
...
add_action( 'wpcf7_admin_init', 'wpcf7cms_add_tag_generator_number', 18, 0 );

function wpcf7cms_add_tag_generator_number() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'number', __( 'number', 'contact-form-7' ),
		'wpcf7cms_tag_generator_number' );
}

function wpcf7cms_tag_generator_number( $contact_form, $args = '' ) {
